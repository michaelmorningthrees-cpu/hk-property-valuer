...
async function scrapeCitibankValuation(propertyData) {
  ...
  try {
    ...
    // 先處理 Cookie Banner（避免覆蓋）
    try {
      const cookieBtn = page.locator('#onetrust-accept-btn-handler, button:has-text("Accept"), button:has-text("同意"), button:has-text("接受"), button:has-text("Close")').first();
      if (await cookieBtn.isVisible()) {
        console.log('🍪 [Citi] 關閉 Cookie Banner...');
    ...
    const waitReady = async (selector) => {
      ...
      try {
        ...
        console.warn(`⚠️ [Citi] 等待 ${selector} 逾時，重試一次...`);
    ...
    console.log('🔘 [Citi] 準備點擊 (嚴格模式)...');

    await page.evaluate(() => {
      const targets = Array.from(document.querySelectorAll('a, button, input[type="button"], div[role="button"]'));
      const btn = targets.find(el => (el.innerText || '').includes('進行物業估價'));
      if (btn) {
    ...
    try {
      await page.getByRole('button', { name: /進行物業估價/ }).click({ force: true, timeout: 2000 });
    } catch (e) {
      // ignore
    }

    console.log('⏳ [Citi] 正在捲動頁面以觸發 Lazy Loading...');
    ...
    for (let i = 0; i < 20; i += 1) {
      ...
      const isVisible = await page.evaluate(() => {
        ...
        return text.includes('參考編號') || text.includes('Reference No') || (text.includes('估價') && text.includes('港幣'));
      });

      if (isVisible) {
        console.log('✅ [Citi] 捲動後發現結果！');
        foundResult = true;
        break;
      }
    }
    ...
    if (foundResult) {
      console.log('⏳ [Citi] 結果標籤已出現，正在等待價格數字渲染...');
      try {
        await page.waitForFunction(() => {
          const text = document.body.innerText || '';
          return /\d{1,3},\d{3}(?:,\d{3})?/.test(text);
        }, { timeout: 10000 });
        console.log('✅ [Citi] 價格數字已渲染！');
      } catch (e) {
        console.warn('⚠️ [Citi] 等待價格數字超時 (可能已存在或無法識別)');
      }
    } else {
      console.error('❌ [Citi] 捲動到底仍未見結果 (或系統繁忙)');
      await page.screenshot({ path: 'citi-scroll-fail.png', fullPage: true });
    }

    console.log('💰 [Citi] 開始提取價格...');
    let price = null;
    let bodyText = '';

    try {
      bodyText = await page.evaluate(() => document.body.innerText || '');
      const matches = bodyText.match(/\b\d{1,3}(?:,\d{3})+(?:\.\d+)?\b/g);

      if (matches) {
        console.log(`   [Debug] 找到的所有數字: ${JSON.stringify(matches)}`);
        const numbers = matches
          .map(s => Number(s.replace(/,/g, '')))
          .filter(n => n > 800000 && n < 1000000000);
      ...
      console.error('   ⚠️ 提取過程發生錯誤:', e.message);
    }

    if (price) {
      console.log(`💰 [Citi] 最終提取價格: ${price}`);
      await browser.close();
      return price;
    }

    console.error('❌ [Citi] 提取失敗 (Debug: 保存 HTML)');
    const html = await page.content();
    fs.writeFileSync('citi-debug.html', html);
    await browser.close();
    return null;
  } catch (error) {
